...
public final class GDGaugeView: UIView {
    ...
    public func setupView(){
        ...
        if containerBorderWidth == nil {
            containerBorderWidth = 50
        }
        if containerColor == nil {
            containerColor = UIColor(red: 255 / 255, green: 255 / 255, blue: 255 / 255, alpha: 0)
        }
        if handleColor == nil {
            handleColor = UIColor(red: 37 / 255, green: 48 / 255, blue: 74 / 255, alpha: 1)
        }
     ...
     */
    public func setupContainer(width: CGFloat = 14,
                               color: UIColor = UIColor(red: 47 / 255, green: 0 / 255, blue: 0 / 255, alpha: 0),
                               handleColor: UIColor = UIColor(red: 255 / 255, green: 255 / 255, blue: 255 / 255, alpha: 1),
                               shouldShowContainerBorder: Bool = true,
                               shouldShowFullCircle: Bool = false,
                               indicatorsColor: UIColor = UIColor(red: 255 / 255, green: 255 / 255, blue: 255 / 255, alpha: 0),
                               indicatorsValuesColor: UIColor = UIColor(red: 203 / 255, green: 203 / 255, blue: 203 / 255, alpha: 1),
                               indicatorsFont: UIFont = UIFont.systemFont(ofSize: 12)) -> Self {
        
        self.containerBorderWidth = 16
    ...
    public func updateColors(containerColor: UIColor, indicatorsColor: UIColor) {
        ...
        UIView.animate(withDuration: 0.4) {
            ...
            self.handleShape.fillColor = indicatorsColor.cgColor
            self.handleShape.lineCap = .round
...
//                $0.strokeColor = indicatorsColor.cgColor
    ...
    public func updateFontColor(fontColor: UIColor) {
        
       self.indicatorsValuesColor = fontColor
    }
...
//                $0.strokeColor = self.indicatorsColor.cgColor
    ...
    fileprivate func drawHandleShape() {
        ...
        let endVal = startVal + length
        let centerPoint = CGPoint(x: frame.width, y: frame.height)
    ...
    fileprivate func drawIndicators() {
        var center = CGPoint(x: frame.width / 1.82, y: frame.height / 1.82)
        
        if UIScreen.main.bounds.height < 668 {
            center = CGPoint(x: frame.width / 2, y: frame.height / 2)
        } else if UIScreen.main.bounds.height < 813 {
            center = CGPoint(x: frame.width / 2, y: frame.height / 1.92)
        }else {
            center = CGPoint(x: frame.width / 1.82, y: frame.height / 1.82)
        }
...
//        addUnitIndicator(centerPoint: center)
    ...
    fileprivate func addTextLabels(centerPoint: CGPoint) {
        for i in 0...totalSeparationPoints {
            //to change the indicator values position
            var width = (frame.width / 3) * 1
            if UIScreen.main.bounds.height < 668 {
                width = (frame.width / 3.26) * 1
            } else {
            ...
            let xOffset = abs(cos(baseRad)) * size.width * 0.0
            let yOffset = abs(sin(baseRad)) * size.height * 0.0
