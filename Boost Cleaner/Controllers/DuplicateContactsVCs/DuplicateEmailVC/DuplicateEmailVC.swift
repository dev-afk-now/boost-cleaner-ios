...
class DuplicateEmailVC: UIViewController ,CNContactViewControllerDelegate{
    ...
    var  duplicatesDataArryEmail = [DuplicatesModel]()
    ...
    @objc func backgroundthread() {
        ...
        duplicatesDataArryEmail.removeAll()
        print( mainDataArryEmail.count)
        for i in 0..<mainDataArryEmail.count {
            DispatchQueue.main.async {
                let value2 = Double(i)
                self.circleProgressView.progress = Double(value2/Double(self.mainDataArryEmail.count))
                let x = Double(value2/Double(self.mainDataArryEmail.count))
                let y = Double(round(100*x)/100)
                let result = Int(y * 100)
                self.percentageLbl.text = "\(result)" + "%"
            }
            mergeAllDuplicates(ContactArray: mainDataArryEmail[i].ContactArray!)
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        self.title = "Duplicate Email".localized
    ...
    @objc func selectAllbtn() {
        ...
        if(flagselect){
            ...
            if(selectedRows.isEmpty){
                ...
                mergebtn.isHidden = false
                self.user.deleteValue = (selectedRows.count  - mainDataArryEmail.count )
                let txt = "MERGE SELECTED".localized
                mergebtn.setTitle("\(txt) (" + "\(selectedRows.count  - mainDataArryEmail.count )" + ")", for: .normal)
    ...
    func getAllIndexPathsSection() -> [Int] {
        var indexPaths: [Int] = []
        for i in 0..<duplicatesDataArryEmail.count {
            indexPaths.append(i)
        }
        return indexPaths
    }
    ...
    func mergeAllDuplicates(ContactArray : [CNContact]) {
        ...
        for item in duplicates {
            ...
            self.duplicatesDataArryEmail.append(DuplicatesModel( ContactArray :ContactArray, MergeArray: newContact ))
    ...
    //        let title = "Allow Merge"
    //        let message = "Are you sure you want to merge these contacts?"
    //        let alert = AlertView.prepare(title: title, action1: "Merge", action2: "Cancel", message: message, actionOne: {
    //            self.confirmedMerge()
    //        }, actionTwo: {
    //            // Cancel Event
    //            print("Cancel Event")
    ...
    @IBAction func mergeBtnAction(_ sender: UIButton) {
        ...
        if (PaymentManager.shared.isPurchase() == false) && inAppSpot.contains(InAppEventLocations.onContactsDuplicateEmail) {
            Utilities.showPremium(parentController: self, isAppDelegate: false, isFromStore: false, iapEvent: InAppEventLocations.onContactsDuplicateEmail)
        } else {
            deleteAlert()
        }
    }
    ...
    func confirmedMerge() {
        for k in 0..<selectedRowsSection.count  {
            for i in 0..<duplicatesDataArryEmail[selectedRowsSection[k]].ContactArray!.count  {
                
                let contact = duplicatesDataArryEmail[selectedRowsSection[k]].ContactArray![i]
        ...
        for k in 0..<selectedRowsSection.count  {
            ...
            let contact = duplicatesDataArryEmail[selectedRowsSection[k]].MergeArray!
            let mutableContact = contact.mutableCopy() as! CNMutableContact
...
extension DuplicateEmailVC : UITableViewDelegate, UITableViewDataSource {
    
    func numberOfSections(in tableView: UITableView) -> Int {
        
        return duplicatesDataArryEmail.count
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        return  duplicatesDataArryEmail[section].ContactArray!.count
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var dataArry = duplicatesDataArryEmail[indexPath.section].ContactArray![indexPath.row]
        ...
        if(duplicatesDataArryEmail[indexPath.section].ContactArray!.count == indexPath.row+1){
            cell.maincellView.isHidden = false
            cell.maincellView.layer.roundCorners([.leftBottom,.rightBottom], corner: 15)
            
        }
        
        if(dataArry.emailAddresses.first?.value as String? == nil){
            cell.numberlbl.text = "No Email".localized
        }
        else{
            cell.numberlbl.text = dataArry.emailAddresses.first?.value as String?
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        DispatchQueue.main.async {
            ...
            var contact = self.duplicatesDataArryEmail[indexPath.section].ContactArray![indexPath.row]
            ...
            backbtn.setImage(UIImage.init(named: "back_btn_ic"), for: UIControl.State.normal)
            backbtn.addTarget(self, action:#selector(self.doneButtonClicked), for: UIControl.Event.touchUpInside)
            backbtn.frame = CGRect.init(x: 0, y: 0, width: 25, height: 24)
            let barButton = UIBarButtonItem.init(customView: backbtn)
            contactViewController.navigationItem.backBarButtonItem = barButton
            ...
            //    UINavigationBar.appearance().titleTextAttributes = [NSAttributedString.Key.foregroundColor:  UIColor(red: 21/255, green: 85/255, blue: 214/255, alpha: 1.0)]
            //  self.navigationController?.navigationBar.tintColor = UIColor(red: 21/255, green: 85/255, blue: 214/255, alpha: 1.0)
            if #available(iOS 13.0, *) {
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        var dataArry = duplicatesDataArryEmail[section].MergeArray!
        ...
        headerCell.maincellView2.dropShadow()
        ...
        if(dataArry.emailAddresses.first?.value as String? == nil){
            headerCell.numberlbl.text = "No Email".localized
        }
        else{
            headerCell.numberlbl.text = dataArry.emailAddresses.first?.value as String?
    ...
    @objc func headerSelection(_ sender:UIButton)
    {
        ...
        DispatchQueue.main.async {
            self.title = ""
            self.navigationItem.setHidesBackButton(false, animated: false)
            var contact = self.duplicatesDataArryEmail[selectedIndexPath].MergeArray!
            if !contact.areKeysAvailable([CNContactViewController.descriptorForRequiredKeys()]) {
                do {
                    contact = try self.contactStore.unifiedContact(withIdentifier: contact.identifier, keysToFetch: [CNContactViewController.descriptorForRequiredKeys()])
                }
...
extension DuplicateEmailVC: AlterDeleteDelegate {
