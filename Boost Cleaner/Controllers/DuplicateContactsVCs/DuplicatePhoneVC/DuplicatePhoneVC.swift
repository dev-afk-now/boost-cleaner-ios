...
class DuplicatePhoneVC: UIViewController ,CNContactViewControllerDelegate{
    ...
    var  duplicatesDataArryPhone = [DuplicatesModel]()
    ...
    @objc func backgroundthread() {
        duplicatesDataArryPhone.removeAll()
        
        
        for i in 0..<mainDataArryPhone.count {
            DispatchQueue.main.async {
                let value2 = Double(i)
                self.circleProgressView.progress = Double(value2/Double(self.mainDataArryPhone.count))
                let x = Double(value2/Double(self.mainDataArryPhone.count))
                let y = Double(round(100*x)/100)
                let result = Int(y * 100)
                self.percentageLbl.text = "\(result)" + "%"
            }
            mergeAllDuplicates(ContactArray: mainDataArryPhone[i].ContactArray!)
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        self.title = "Duplicate Phone".localized
    ...
    @objc func selectAllbtn() {
        ...
        if(flagselect){
            ...
            if(selectedRows.isEmpty){
                ...
                self.user.deleteValue = (selectedRows.count   - mainDataArryPhone.count )
                let txt = "MERGE SELECTED".localized
                mergebtn.setTitle("\(txt) (" + "\(selectedRows.count  - mainDataArryPhone.count )" + ")", for: .normal)
            }
    ...
    func getAllIndexPathsSection() -> [Int] {
        var indexPaths: [Int] = []
        for i in 0..<duplicatesDataArryPhone.count {
            indexPaths.append(i)
        }
        return indexPaths
    }
    ...
    @IBAction func mergeBtnAction(_ sender: UIButton) {
        ...
        if (PaymentManager.shared.isPurchase() == false) && inAppSpot.contains(InAppEventLocations.onContactsDuplicatePhone) {
            Utilities.showPremium(parentController: self, isAppDelegate: false, isFromStore: false, iapEvent: InAppEventLocations.onContactsDuplicatePhone)
        } else {
            deleteAlert()
        }
    }
    ...
    func confirmedMerge() {
        //   mergeAllDuplicates()
        //  print(MergeNameVC.merge(duplicates: ContactArray))
        
        
        for k in 0..<selectedRowsSection.count  {
            for i in 0..<duplicatesDataArryPhone[selectedRowsSection[k]].ContactArray!.count  {
                
                let contact = duplicatesDataArryPhone[selectedRowsSection[k]].ContactArray![i]
        ...
        for k in 0..<selectedRowsSection.count  {
            ...
            let contact = duplicatesDataArryPhone[selectedRowsSection[k]].MergeArray!
            let mutableContact = contact.mutableCopy() as! CNMutableContact
    ...
    func mergeAllDuplicates(ContactArray : [CNContact]) {
        ...
        for item in duplicates {
            ...
            self.duplicatesDataArryPhone.append(DuplicatesModel( ContactArray :ContactArray, MergeArray: newContact ))
...
extension DuplicatePhoneVC : UITableViewDelegate, UITableViewDataSource {
    
    func numberOfSections(in tableView: UITableView) -> Int {
        
        return duplicatesDataArryPhone.count
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        return  duplicatesDataArryPhone[section].ContactArray!.count
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var dataArry = duplicatesDataArryPhone[indexPath.section].ContactArray![indexPath.row]
        ...
        //        if(duplicatesDataArryPhone[indexPath.section].ContactArray!.count == indexPath.row+1){
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        DispatchQueue.main.async {
            ...
            var contact = self.duplicatesDataArryPhone[indexPath.section].ContactArray![indexPath.row]
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        var dataArry = duplicatesDataArryPhone[section].MergeArray!
        ...
        if(dataArry.phoneNumbers.first?.value.stringValue == nil){
            headerCell.numberlbl.text = "No phone"
        }
        else{
    ...
    @objc func headerSelection(_ sender:UIButton)
    {
        ...
        DispatchQueue.main.async {
            self.title = ""
            self.navigationItem.setHidesBackButton(false, animated: false)
            var contact = self.duplicatesDataArryPhone[selectedIndexPath].MergeArray!
            if !contact.areKeysAvailable([CNContactViewController.descriptorForRequiredKeys()]) {
                do {
                    contact = try self.contactStore.unifiedContact(withIdentifier: contact.identifier, keysToFetch: [CNContactViewController.descriptorForRequiredKeys()])
                }
...
extension DuplicatePhoneVC: AlterDeleteDelegate {
