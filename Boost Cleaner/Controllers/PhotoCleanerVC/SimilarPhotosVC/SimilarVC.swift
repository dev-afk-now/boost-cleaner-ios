...
class SimilarVC: UIViewController {
    
    @IBOutlet weak var btnTrash: UIButton!
    @IBOutlet weak var btnKeep: UIButton!
    @IBOutlet weak var viewHeaderBack: UIView!
    @IBOutlet weak var constraintHeaderHeight: NSLayoutConstraint!
    @IBOutlet weak var fastImgActionControl: UIControl!
    @IBOutlet weak var lblFastClean: UILabel!
    @IBOutlet weak var imgvFastCleanGif: UIImageView!
    @IBOutlet weak var lblTitleSimilar: UILabel!
    ...
    func setTitleAndCount() {
        lblTitleSimilar.text = "Similar".localized
        lblPhotosCount.text = "\(photosArray.count)" + " " + "Photos".localized
    }
    
    func showHeader() {
        if (PaymentManager.shared.isPurchase() == false) {
            viewHeaderBack.isHidden = false
            constraintHeaderHeight.constant = 60
            loadAnimationGif()
        } else {
            viewHeaderBack.isHidden = true
            constraintHeaderHeight.constant = 0
            self.imgvFastCleanGif.image = nil
        }
        self.view.layoutIfNeeded()
    }
    
    func loadAnimationGif() {
        self.imgvFastCleanGif.image = nil
        do {
            let gif = try UIImage(gifName: "ic_fast_gif.gif")
            self.imgvFastCleanGif.setGifImage(gif, loopCount: -1)
        } catch {
            print(error)
        }
    }
    
    @IBAction func onBtnFastCleanAction(_ sender: UIControl) {
        Utilities.showPremium(parentController: self, isAppDelegate: false, isFromStore: false, iapEvent: InAppEventLocations.onSimilarFastClean)
    }
    ...
    @objc func backClick2() {
        ...
        for i in 0..<self.arrayIndxPath.count {
            self.user.finalarrPhotoAssetsData[self.indArray].assetResult.remove(at: self.arrayIndxPath[i])
        }
        if( self.user.finalarrPhotoAssetsData[self.indArray].assetResult.count <= 1){
            self.user.finalarrPhotoAssetsData.remove(at: self.indArray)
        }
            ...
            if(success) {
                ...
                DispatchQueue.main.async { [self] in
                    ...
                    for i in 0..<self.similarphotos.count {
                        print( self.similarphotos[i].NumberArry!.count)
...
extension SimilarVC: UITableViewDelegate,UITableViewDataSource , UIScrollViewDelegate{
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        self.currentIndexstop = indexPath.row
        ...
        self.isScrollingend = true
        self.isScrollingend = false
        ...
        timer.invalidate()
        timer = Timer.scheduledTimer(withTimeInterval: 0.20, repeats: false) { (timer) in
            self.isScrollingend = true
            tableViewCell.setCollectionViewDataSourceDelegate(dataSourceDelegate: self, forRow: indexPath.row)
        ...
        if EventManager.shared.dailyLimit == true && PaymentManager.shared.isPurchase() == false {
            ...
            let okAction = UIAlertAction(title: "\(deleteStr) (" + "\(selectedRows.count)" + ")", style: .default) { (action) in
                if(leftCount >= self.selectedRows.count){
                    ...
                    if (PaymentManager.shared.isPurchase() == false) && inAppSpot.contains(InAppEventLocations.onPhotoSimilarDelete) {
                        Utilities.showPremium(parentController: self, isAppDelegate: false, isFromStore: false, iapEvent: InAppEventLocations.onPhotoSimilarDelete)
                    } else {
                        self.photosDelete()
                    }
                }
...
extension SimilarVC:  UICollectionViewDelegate,UICollectionViewDataSource  ,UICollectionViewDelegateFlowLayout{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if(isScrollingend){
            autoreleasepool {
                ...
                SimilarVC.getAssetThumbnail(asset: asset) { [self] (result, info) in
...
extension Array {
    ...
    //    mutating func removepath(at indexes: [IndexPath]) {
    //      for index2 in indexes.sorted(by: >) {
    //        remove(at: index2)
    //      }
    //    }
}
