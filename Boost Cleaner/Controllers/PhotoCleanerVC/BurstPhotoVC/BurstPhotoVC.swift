...
class BurstPhotoVC: UIViewController {
    
    var photosArray: PHFetchResult<PHAsset>?
    var burstresultArray = [[NSString]]()
    var photos = [PHAsset]()
    var deletephotos = [PHAsset]()
    let user = SharedData.sharedUserInfo
    var totalcount = 0
    var burstphotosArray = [PhotosModel]()
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.title = "Burst Photos"
        self.leftBarCloseButtonItems(iconName: "back_btn_ic")
        self.makeNavigationBarTransparent()
        
         autoreleasepool {
             for k in 0..<burstresultArray.count {
                 var numberData = [Int]()
                 for l in 0..<burstresultArray[k].count {
                     let number = Int(burstresultArray[k][l] as String)
                     numberData.append(number!)
                 }
                 if(!numberData.isEmpty){
                     self.burstphotosArray.append(PhotosModel( title: "\(burstresultArray[k].count)", NumberArry: numberData, firstPhoto: numberData[0] ) )
                 }
             }
         }
...
extension BurstPhotoVC:UICollectionViewDelegate,UICollectionViewDataSource{
    
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return burstphotosArray.count
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let asset =    self.photosArray![burstphotosArray[indexPath.row].firstPhoto!]
        cell.configureCell(phAsset: asset)
        cell.countLbl.text = burstphotosArray[indexPath.row].title
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath){
        ...
        var photosArray2 = [PhotosModel]()
        photosArray2.removeAll()
        photosArray2.append(burstphotosArray[indexPath.row])
//        let vc = UIStoryboard.init(name: "Main", bundle: Bundle.main).instantiateViewController(withIdentifier: "SimilarVC") as? SimilarVC
//        vc!.similarphotos = photosArray2
...
extension BurstPhotoVC: PinterestLayoutDelegate {
